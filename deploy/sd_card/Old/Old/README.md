...
## Launcher + Dual Kernel Dev Mode (Thonny/PC)

This workspace now includes a terminal-first bootstrapper for rapid development in Thonny:

- `main.py`: launcher menu (choose game kernel)
- `guild_sim_kernel.py`: Isekai Guild Sim training/class economy prototype
- `sw25_kernel.py`: Sword World 2.5 text runner prototype
- `pc_menu.py`: keyboard menu (`w`, `s`, `enter`, `q`)
- `sd_setup.py`: Pico SD mount plus desktop fallback
- `storage.py`: JSON save/load helper

### Run On PC In Thonny

1. Open `main.py`
2. Run with Python
3. Use `w`, `s`, and Enter in the shell prompt

Save files are written to:

- `sd/...` when SD fallback is used, or
- `data/...` if no SD path is available

## Pico LCD BMP Viewer (24-bit)

If you want sprite or image rendering on the Waveshare 1.8in LCD without high RAM usage,
use the new row-streaming BMP path:

- `bmp24_stream.py`: low-memory 24-bit BMP parser
- `bmp_viewer_demo.py`: loops through `/images/*.bmp` and displays each image

Requirements:

1. Put `st7735_1inch8.py`, `bmp24_stream.py`, and `bmp_viewer_demo.py` on the Pico.
2. Create `/images` on the Pico filesystem.
3. Export source images as **24-bit BMP** (for example from GIMP).
4. Copy `.bmp` files into `/images`.
5. Run `bmp_viewer_demo.py`.

If red and blue appear swapped, set `USE_BGR565 = True` in `bmp_viewer_demo.py`.

### Run On Pico W

1. Copy these files to the Pico:
  - `main.py`
  - `pc_menu.py`
  - `sd_setup.py`
  - `storage.py`
  - `guild_sim_kernel.py`
  - `sw25_kernel.py`
2. Add campaign JSON files in `sw25/campaigns` (or SD path `/sd/sw25/campaigns`)
3. Reboot Pico and run `main.py`

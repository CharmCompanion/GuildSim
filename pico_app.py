...
from pico_assets import draw_scene_background, draw_enemy_asset, draw_ui_icon
...
class PicoGuildSim:
    ...
    DASHBOARD_TARGETS = ("roster", "tavern", "missions", "training", "settings", "seeds", "log")
    DASHBOARD_NAV = ("Roster", "Tavern", "Quest", "Train", "Menu", "Seeds", "Log")
    ...
    def _ensure_missions(self):
        if self.slot not in self.available_missions or len(self.available_missions[self.slot]) == 0:
            custom = list(self.guild.get("custom_adventure_seeds", []))
            custom.extend(self._load_builder_custom_seeds())
            self.available_missions[self.slot] = get_available_missions(self.guild["rank"], custom)

    def _load_builder_custom_seeds(self):
        # Desktop builder writes here; on Pico this may not exist, so fail soft.
        path = "assets/layouts/custom_adventures.json"
        try:
        ...
        if isinstance(data, list):
            for e in data:
                code = e.get("seed_code", "") if isinstance(e, dict) else ""
                if code:
                    out.append(code)
        return out

    def _load_seed_library_entries(self):
        path = "assets/layouts/seed_library.json"
        try:
            with open(path, "r") as f:
                data = json.load(f)
        except Exception:
    ...
    def _ensure_preloaded_seeds(self):
        self.preloaded_seed_entries = self._load_seed_library_entries()

    def _draw_bg(self):
        if not draw_scene_background(self.d, self.scene):
            self.d.fill(COL_BG)
        # Thin bars: header + nav/footer strip
        self.d.fill_rect(0, 0, 128, 12, 0x0841)
        self.d.fill_rect(0, 152, 128, 8, 0x0841)

    def _draw_header(self, title):
        self.d.text(title[:16], 2, 3, COL_ACCENT)
        if self.guild:
            g = str(self.guild.get("gold", 0))
            if not draw_ui_icon(self.d, "gold", 72, 2, 8, 8):
                self.d.text("G", 72, 3, COL_TEXT)
            self.d.text(":" + g, 82, 3, COL_TEXT)

    def _draw_footer(self):
        # Show prev/current/next scene words instead of tiny abbreviations.
        idx = self.SCENE_ORDER.index(self.scene) if self.scene in self.SCENE_ORDER else 0
        prev_scene = self.SCENE_ORDER[(idx - 1) % len(self.SCENE_ORDER)]
        next_scene = self.SCENE_ORDER[(idx + 1) % len(self.SCENE_ORDER)]
        prev_label = self.SCENE_SHORT[prev_scene][:7]
        cur_label = self.SCENE_SHORT[self.scene][:8]
        next_label = self.SCENE_SHORT[next_scene][:7]

        self.d.fill_rect(0, 150, 128, 10, 0x0841)
        self.d.text(prev_label, 2, 152, COL_DIM)
        hi = COL_OK if self.scene_switch_mode else COL_ACCENT
        self.d.fill_rect(44, 151, 40, 8, hi)
        self.d.text(cur_label, 46, 152, COL_BG)
        self.d.text(next_label, 92, 152, COL_DIM)
    ...
    def _draw_list_cursor(self, row, y0=18, step=12):
        y = y0 + row * step
        self.d.fill_rect(1, y - 1, 126, 10, 0x2104)
        self.d.rect(1, y - 1, 126, 10, COL_CURSOR)

    def _draw_bar_row(self, y, active):
        fill = 0x2104 if active else 0x0841
        border = COL_CURSOR if active else COL_DIM
        self.d.fill_rect(2, y - 1, 124, 10, fill)
        self.d.rect(2, y - 1, 124, 10, border)
    ...
    def render_save_slots(self):
        ...
        for i, s in enumerate(slots):
            y = 20 + i * 14
            self._draw_bar_row(y, i == self.cursor)
    ...
    def render_dashboard(self):
        ...
        # Replace vertical text list with readable horizontal bars.
        for i, name in enumerate(self.DASHBOARD_NAV):
            y = 90 + i * 9
            active = i == self.cursor
            fill = 0x2104 if active else 0x0841
            border = COL_CURSOR if active else COL_DIM
            text_col = COL_TEXT if active else COL_DIM
            self.d.fill_rect(2, y, 124, 8, fill)
            self.d.rect(2, y, 124, 8, border)
            self.d.text(name, 6, y + 1, text_col)
    ...
    def render_roster(self):
        ...
        for i, r in enumerate(vis):
            y = 20 + i * 12
            self._draw_bar_row(y, i == (self.cursor - top))
    ...
    def render_tavern(self):
        ...
        for i, r in enumerate(vis):
            y = 20 + i * 12
            row_idx = top + i
            self._draw_bar_row(y, self.cursor == row_idx)
            draw_recruit(self.d, 3, y - 2, r, scale=1)
            self.d.text(r["name"][:7], 20, y, COL_TEXT)
            self.d.text(str(r["hire_cost"]) + "g", 84, y, COL_ACCENT)
        y = 20 + len(vis) * 12
        self._draw_bar_row(y, self.cursor == max_idx)
        label = "[Refresh 20g]"
    ...
    def render_missions(self):
        ...
        party = self._active_party()
        draw_ui_icon(self.d, "party", 2, 18, 8, 8)
        ...
        if missions and self.cursor < len(missions):
            mission = missions[self.cursor]
            self.d.fill_rect(92, 106, 34, 38, 0x2104)
            self.d.rect(92, 106, 34, 38, COL_CURSOR)
            if not draw_enemy_asset(self.d, mission, 95, 110, 28, 24):
                self.d.text("EN", 102, 120, COL_BAD)
            self.d.text("D" + str(mission.get("difficulty", 1)), 100, 136, COL_TEXT)
            seed = mission.get("seed_code", "")
            self.d.fill_rect(2, 142, 124, 8, 0x2104)
            self.d.text(("Seed " + seed)[0:20], 4, 143, COL_DIM)
    ...
    def render_training(self):
        ...
        for i, cat in enumerate(cats):
            ...
            self.d.text(cat[:7], 4, y, COL_TEXT)
            self.d.text(status[:8], 62, y, COL_OK if unlocked else COL_DIM)
    ...
    def render_settings(self):
        ...
        opts = ["Rename Guild", "Rename Lord", "Add Seed", "Open Seeds", "Quit Save"]
        for i, o in enumerate(opts):
            y = 20 + i * 12
            self._draw_bar_row(y, i == self.cursor)
            self.d.text(o[:15], 4, y, COL_TEXT)
        self.d.text(("Guild " + self.guild["name"])[:20], 2, 64, COL_DIM)
        self.d.text(("Lord " + self.guild["lord_name"])[:20], 2, 74, COL_DIM)
        count = len(self.guild.get("custom_adventure_seeds", []))
        self.d.text(("Seeds " + str(count) + " shared")[:20], 2, 86, COL_DIM)
        self.d.text("A cycles/adds seed", 2, 96, COL_DIM)

    def render_seeds(self):
        self._draw_header("Seed Library")
        self._ensure_preloaded_seeds()
        entries = self.preloaded_seed_entries[:10]
        if not entries:
            self.d.text("No preloaded", 4, 22, COL_DIM)
            return
        ...
        for i, e in enumerate(entries):
            y = 20 + i * 12
            self._draw_bar_row(y, i == self.cursor)
            label = (e.get("kind", "seed")[:1].upper() + ":" + e.get("title", "Seed")[:8])
            self.d.text(label, 4, y, COL_TEXT)
            self.d.text(e.get("rating", "G")[:2], 96, y, COL_DIM)

        sel = entries[self.cursor]
        code = str(sel.get("seed_code", ""))
        self.d.fill_rect(2, 140, 124, 10, 0x2104)
        self.d.text(code[:20], 4, 141, COL_DIM)
    ...
    def _handle_dashboard(self, events):
        max_row = len(self.DASHBOARD_TARGETS) - 1
        if events["move"] == "up":
            self.cursor = max(0, self.cursor - 1)
        elif events["move"] == "down":
    ...
    def _handle_missions(self, events):
        ...
        if events["a_pressed"]:
            if self.cursor == max_idx:
                custom = self.guild.get("custom_adventure_seeds", [])
                self.available_missions[self.slot] = get_available_missions(self.guild["rank"], custom)
                self.cursor = 0
    ...
    def _handle_settings(self, events):
        ...
        if events["a_pressed"]:
            if self.cursor == 0:
                ...
                # Cycles a small built-in set of shared seed examples and stores in save.
                bank = [
                    "ADV-4Y44D0NHNYM0RP",
                    "ADV-4Y9W0EYSHDF4W7",
                    "ADV-4YAUWT9L4XQOMQ",
                ...
                seeds = self.guild.setdefault("custom_adventure_seeds", [])
                if len(seeds) < len(bank):
                    nxt = bank[len(seeds)]
                else:
                    nxt = bank[0]
                if decode_adventure_seed_code(nxt):
                    if nxt not in seeds:
                        seeds.append(nxt)
                    self.save_current()
                    self.available_missions[self.slot] = []
                    self.flash("Seed added")
                else:
                    self.flash("Seed invalid")
            elif self.cursor == 3:
                self.scene = "seeds"
                self.cursor = 0
            else:
        ...
        if events["a_pressed"]:
            ...
            if code.startswith("ADV-"):
                ...
                if code not in seeds:
                    ...
                    self.save_current()
                    self.available_missions[self.slot] = []
                    self.flash("Added adv seed")
                else:
                    self.flash("Already added")
            else:
                self.flash("Place seed only")
